...
export const newPuppy = async (_req: Request, res: Response) => {
    try {
        const newPuppy = await Puppy.create({ name: _req.body.name, breed: _req.body.breed, birthday: _req.body.birthday})
...
export const updatePuppies = async (_req: Request, res: Response) => {
    try {
        if(!_req.body.name || !_req.body.breed || !_req.body.birthday) return res.status(400).json({error: "Missing properties"})
